...
def clean_data(data):
    ...
    y = np.array([0 if i < 0 else int(i * 1000) for i in data_y.iloc[:, 0]])
...
def model_train(x_train, x_test, y_train, y_test, x_now):
    ...
    score = etree.score(x_test, y_test)
    print("平均损失度为：", score)
    y_predict = etree.predict(x_test)
    loss = mean_squared_error(y_test, y_predict)
    print('损失为', loss)
    predict = etree.predict(x_now.reshape(1, len(x_now)))
    print("预测结果为", predict)
...
def main():
    ...
    final_result.to_csv('E_result_20.csv', encoding='utf-8')
    print(final_result)
